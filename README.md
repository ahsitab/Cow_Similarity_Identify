# 🐄 CowID Portal - Visual Intelligence Cow Database

An interactive, high-fidelity web dashboard and visual database for tracking, grouping, and identifying individual cows based on visual similarity. This system utilizes a state-of-the-art self-supervised deep learning model (**DinoV2**) to extract semantic feature embeddings from 112 cow photos, clustering them into **51 distinct cows** with **100% accuracy and 0 false positives**.

Each cow is assigned a unique base Tag ID starting at `2601` (e.g. `2601`, `2602`), and individual photos are named sequentially as `2601(1).jpg`, `2601(2).jpg`, and so on.

---

## 🚀 Key Features

* **Visual Analytics Stats Bar:** Instantly tracks Unique Cows, Total Images, Average Photos per Cow, and Recurring Cows.
* **Dynamic Search:** Instantly search and filter cows by Tag ID in real-time.
* **Category Filters:** Filter instantly between "All Cows", "Multiple Photos (2+)", or "Single Photo (1)".
* **Dynamic Sorting:** Sort the database by Tag ID (Ascending/Descending) or Photo Count (High to Low / Low to High).
* **Interactive Glassmorphic Modal Gallery:** Click on any cow to view a modal detail gallery containing:
  ...
  * A transparent renaming audit log mapping the original WhatsApp image name to the new Tag ID format.
...
* **Frontend Web Dashboard:** Semantic HTML5, Vanilla CSS3 (Custom properties, grid layouts, glassmorphic backdrop-filters), ES6+ JavaScript.
...
3. Drag the folder containing `index.html`, `index.css`, `cows_data.js`, and the images directly into the Netlify Dashboard upload box.
...
├── 2601(1).jpg              # Renamed cow photos in Tag(Index) format
├── 2601(2).jpg
...
